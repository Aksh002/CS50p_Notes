...
for arg in sys.argv[1:]:    #we use use slice fxn([1:]) with sys.argv as we know that sys.argv() return file name at [0] position in the list
    ...
    images.append(image)
images[0].save(
    "costumes.gif",save_all=True,append_images=images[1:],duration=200,loop=0     #loop=0 signifies looping infinite times
...
#Then we import Image fxn frm PIL library to make changes in the image. Then we take a empty list named images. and run the loop on the sys.argv to open the image and store it in variable named image and append the image in images list
