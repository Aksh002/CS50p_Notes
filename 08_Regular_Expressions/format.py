...
print(f"hellow {name}")
'''
#this wont work if the input doesnt hv the space after coma, to fix that we can use "()". "()" fxn is not only t ogrp things together but also to capture them. Until re.search was used only with if, it was returning True or False. but re.search can also return values if () is usedc in it. it will return whatever is in () in the form of groups.
